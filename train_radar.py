from Backbone import MobileNetV3,customed_backbone,DFCNN,LeNet,Resnet
import os
import torch
import glob
...
def parse_args():
    ...
    parser.add_argument('--data_root', type=str, default="/data/gaoxinjian/datafolder/Sample/Radar/3-IQ-norm", help='path to the dataset')
...
def adjust_learning_rate(optimizer, epoch):
    ...
    lr = args.learn_rate * (0.5 ** (epoch // 50))
...
if __name__ == '__main__':
    ...
    dataloader = {x: torch.utils.data.DataLoader(dataset=eval("{}_set".format(x)), batch_size=args.batch_size, shuffle=True,num_workers = 8) for x in
                ['train', 'test']}
    ...
    if args.use_gpu:
        if args.model == "MobileNetV3":
            ...
            net = DFCNN.DFCNN_ori(num_classes=10)
            model = nn.DataParallel(net)
            model = model.cuda()
            ...
            net = Resnet.ResModel(10)
            model = nn.DataParallel(net)
            model = model.cuda()
            print("**** Training with resnet backbone ****")
    else:
