from Backbone import MobileNetV3,customed_backbone,DFCNN
import os
import torch
import glob
...
def parse_args():
    ...
    parser.add_argument('--data_root', type=str, default="/home/gaoxinjian/datafolder/Sample/IFF/3-IQ-norm", help='path to the dataset')
...
def adjust_learning_rate(optimizer, epoch):
    ...
    lr = args.learn_rate * (0.1 ** (epoch // 20))
    ...
    for e in range(epoch):
        ...
        for phrase in ['train', 'test']:
            ...
            if epoch_acc > max_val_acc and phrase=='test':
                ...
                if file:
                    os.remove(file[0])
                torch.save(model.state_dict(),"./out_put_model/model_acc_{:.4f}.pt".format(epoch_acc))
...
if __name__ == '__main__':
    ...
    dataloader = {x: torch.utils.data.DataLoader(dataset=eval("{}_set".format(x)), batch_size=args.batch_size, shuffle=True) for x in
                ['train', 'test']}
    ...
    if args.use_gpu:
        if args.model == "MobileNetV3":
            ...
            net = DFCNN.DFCNN(num_classes=40)
            model = nn.DataParallel(net)
            model = model.cuda()
